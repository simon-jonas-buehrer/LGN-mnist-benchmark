...
class Net:
    ...
    def __init__(self, n_in: int, widths: list[int], classes: int, codebook: int = 0):
        ...
        # per-gate source count: gates in layer L read from the previous layer's width (n_in for L=0)
        prev_widths = [n_in, *widths[:-1]]
        self.srcmax = jnp.array(
            [pw for pw, w in zip(prev_widths, widths) for _ in range(w)], jnp.int32
        )  # (n_gates,)
    ...
    def model_memory_bytes(self) -> int:
        ...
        if self.codebook:
            ...
            prev_widths = [self.n_in, *self.widths[:-1]]
            wire_bits = sum(
                w * 2 * max(1, math.ceil(math.log2(pw))) for pw, w in zip(prev_widths, self.widths)
            )
    ...
    def _forward(
        ...
        for k in range(len(self.widths)):
            ...
            a, b = prev[sa[lo:hi]], prev[sb[lo:hi]]
            na, nb = ~a, ~b
            prev = (
                ((na & nb) * (c & 1))
                | ((na & b) * ((c >> 1) & 1))
...
def _forward_unpacked(net: Net, tables, wa, wb, X):
    ...
    for k in range(len(net.widths)):
        ...
        a = jnp.take_along_axis(
            prev, wa[:, None, lo:hi].astype(jnp.int32).repeat(prev.shape[1], 1), 2
        )
        b = jnp.take_along_axis(
            prev, wb[:, None, lo:hi].astype(jnp.int32).repeat(prev.shape[1], 1), 2
        )
        sel = (a << 1) | b
        prev = (c >> sel) & 1
...
def selftest() -> None:
    ...
    # the packed (deployed) forward must be bit-identical to the oracle — free AND codebook wiring
    X2 = jax.random.bernoulli(jax.random.PRNGKey(1), 0.5, (256, 64)).astype(jnp.uint8)
    for cb in (0, 8):
        net2 = Net(64, [128, 96, 40], 10, codebook=cb)
        ...
        assert (ref == fast).all(), f"packed forward disagrees with oracle (codebook={cb})"
    print("selftest ok (packed == oracle, for free wiring and codebook wiring)")
